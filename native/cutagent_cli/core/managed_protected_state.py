...
_SEQUENCE_VOLATILE_MODEL_RECORDS = {902: (901, 903), 1253: (1246, 1254)}
    ...
    try:
        ...
        for entry in entries:
            ...
            if (entry["key"] in _SEQUENCE_VOLATILE_TIME_FIELDS
                ...
                value = {"reviewedVolatileTime": True}
...
def _canonical_active_sequence_row(row: dict[str, Any]) -> dict[str, Any]:
    ...
    if isinstance(ui_state, bytes) and len(ui_state) <= 16 * 1024 * 1024:
...
def _database_attestation(conn: Any, affected_ids: set[str], protected_item_ids: set[str]) -> dict[str, Any]:
    ...
    try:
        ...
        fairlight_tracks = {
            ...
            "tracks": [{"Sm2TiTrack_id": row[0], "Type": row[1], "FieldsBlob": row[2]} for row in track_rows],
        ...
        for table in selected:
            ...
            for raw in table_rows:
                ...
                if table == "Sm2TiItem" and row.get("DbType") != "Sm2TiTransition":
                    ...
                    row = _canonical_active_sequence_row(row)

...
const directory=await realpath(await mkdtemp(join(tmpdir(),'davinci-sdk-consumer-')));
const run=(command,args,cwd=directory)=>execFileSync(command,args,{cwd,encoding:'utf8',env:process.env,stdio:['ignore','pipe','pipe']});
const packed=JSON.parse(run('npm',['pack','--json','--workspace','davinci-resolve-sdk','--pack-destination',directory],root))[0];
...
const installed=await realpath(join(directory,'node_modules/davinci-resolve-sdk'));
...
import {CutAgent as DaVinciResolve} from 'davinci-resolve-sdk';
assert.equal(typeof DaVinciResolve.connect,'function');
for(const path of ['actions','schemas','protocol','preview/v0.1'])assert.ok(Object.keys(await import('davinci-resolve-sdk/'+path)).length);
for(const path of ['compatibility.json','compatibility.schema.json','package.json']){
 const {default:value}=await import('davinci-resolve-sdk/'+path,{with:{type:'json'}});
 assert.ok(value);
}
await assert.rejects(import('davinci-resolve-sdk/src/client.js'),{code:'ERR_PACKAGE_PATH_NOT_EXPORTED'});
console.log('All public imports passed; unsupported deep import rejected.');
`);
run(process.execPath,['consumer.mjs']);
await writeFile(join(directory,'consumer.ts'),`import {CutAgent as DaVinciResolve,frames,type ProjectId} from 'davinci-resolve-sdk';
import {ActionIds} from 'davinci-resolve-sdk/actions';
import {ProjectIdSchema} from 'davinci-resolve-sdk/schemas';
import * as protocol from 'davinci-resolve-sdk/protocol';
import * as preview from 'davinci-resolve-sdk/preview/v0.1';
...
const manifest=JSON.parse(await readFile(join(installed,'compatibility.json'),'utf8'));
assert.equal(manifest.packageName,'davinci-resolve-sdk');
const report={directory,package:packed.name,version:packed.version,integrity:packed.integrity,shasum:packed.shasum,files:packed.files.length,publicImports:true,deepImportRejected:true,strictTypes:true,nativeConnectionAttempted:false};
